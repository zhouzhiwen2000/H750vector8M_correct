...
float	Position_KP=3.0,Position_KI=0,Position_KD=25;  //位置控制PID参数
...
void Count_Velocity(void) //用于控制位置模式下的速度 绝对位置模式下，若在执行完成前改变目标，会出问题。
{
    ...
    /*对Bias_X，Y,Z进行Kinematic_Analysis再除以Bias的模然后乘以RC_Velocity相当与对方向为行进方向（Bias_X方向），大小为RC_Velocity的矢量进行Kinematic_Analysis）。*/
    if(relative==0)
    {
