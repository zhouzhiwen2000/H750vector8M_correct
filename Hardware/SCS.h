...
extern int writeWord(uint8_t ID, uint8_t MemAddr, uint16_t wDat, uint8_t End);//写2个字节
...
extern int readWord(uint8_t ID, uint8_t MemAddr, uint8_t End);//读2个字节
extern int Ping(uint8_t ID);//Ping指令

extern void writeBuf(uint8_t ID, uint8_t MemAddr, uint8_t *nDat, uint8_t nLen, uint8_t Fun);
extern void Host2SCS(uint8_t *DataL, uint8_t* DataH, int Data, uint8_t End);//1个16位数拆分为2个8位数
extern int SCS2Host(uint8_t DataL, uint8_t DataH, uint8_t End);//2个8位数组合为1个16位数
